                ...
                <form action="{{ route('pharmacy.inventory.store') }}" method="POST">
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-6">
                            ...
                            <x-text-input id="batch_number" class="block mt-1 w-full form-control" type="text" name="batch_number" :value="old('batch_number')" required placeholder="e.g. BATCH-2023-001" />
                            <x-input-error :messages="$errors->get('batch_number')" class="mt-2" />
                        </div>

                        <div class="col-md-6">
                            <x-input-label for="expiry_date" :value="__('Expiry Date')" />
                            <x-text-input id="expiry_date" class="block mt-1 w-full form-control" type="date" name="expiry_date" :value="old('expiry_date')" required />
                            <x-input-error :messages="$errors->get('expiry_date')" class="mt-2" />
                        </div>

                        <div class="col-md-6">
                            <x-input-label for="quantity_in_stock" :value="__('Quantity Received')" />
                            <x-text-input id="quantity_in_stock" class="block mt-1 w-full form-control" type="number" name="quantity_in_stock" :value="old('quantity_in_stock')" min="1" required />
                        ...
                        <div class="col-md-6">
                            ...
                            <!-- Note: Controller validation says numeric|min:0. Usually this is unit price or total batch price. Assuming Unit Price based on model structure usually, but let's label it simply Purchase Price -->
                             <div class="input-group mt-1">
                                ...
                                <x-text-input id="purchase_price" class="form-control" type="number" step="0.01" name="purchase_price" :value="old('purchase_price')" required />
                            </div>

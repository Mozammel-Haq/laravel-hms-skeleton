    ...
    <div class="container-fluid">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <div>
                ...
                <p class="text-muted mb-0">{{ isset($medicine) ? 'Update medicine details' : 'Register a new medicine in the catalog' }}</p>
            </div>
        ...
        <div class="card border-0 shadow-sm">
            <div class="card-body">
                <form action="{{ isset($medicine) ? route('pharmacy.medicines.update', $medicine) : route('pharmacy.medicines.store') }}" method="POST">
                    @csrf
                    @if(isset($medicine))
                        @method('PUT')
                    @endif
                    ...
                    <div class="row g-3">
                        <div class="col-md-6">
                            ...
                            <x-text-input id="name" class="block mt-1 w-full form-control" type="text" name="name" :value="old('name', $medicine->name ?? '')" required autofocus />
                            <x-input-error :messages="$errors->get('name')" class="mt-2" />
                        </div>

                        <div class="col-md-6">
                            <x-input-label for="generic_name" :value="__('Generic Name')" />
                            <x-text-input id="generic_name" class="block mt-1 w-full form-control" type="text" name="generic_name" :value="old('generic_name', $medicine->generic_name ?? '')" />
                            <x-input-error :messages="$errors->get('generic_name')" class="mt-2" />
                        </div>

                        <div class="col-md-6">
                            <x-input-label for="manufacturer" :value="__('Manufacturer')" />
                            <x-text-input id="manufacturer" class="block mt-1 w-full form-control" type="text" name="manufacturer" :value="old('manufacturer', $medicine->manufacturer ?? '')" />
                        ...
                        <div class="col-md-6">
                            ...
                            <x-text-input id="strength" class="block mt-1 w-full form-control" type="text" name="strength" :value="old('strength', $medicine->strength ?? '')" placeholder="e.g. 500mg" />
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="input-group mt-1">
                                ...
                                <x-text-input id="price" class="form-control" type="number" step="0.01" name="price" :value="old('price', $medicine->price ?? '')" required />
                            </div>
                    ...
                    <div class="mt-4 d-flex justify-content-end">
                        <button type="submit" class="btn btn-primary">
                            <i class="ti ti-device-floppy me-1"></i> {{ isset($medicine) ? 'Update Medicine' : 'Save Medicine' }}
                        </button>
                    </div>
                </form>
            </div>

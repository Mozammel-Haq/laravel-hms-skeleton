                ...
                <form action="{{ route('ipd.store') }}" method="POST">
                    ...
                    <div class="row g-3">
                        <div class="col-md-6">
                            ...
                            @if($patients->isEmpty())
                                <div class="form-text text-warning">
                                    No patients found. <a href="{{ route('patients.create') }}">Register a patient first.</a>
                                </div>
                            @endif
                        </div>
                            ...
                            <select id="doctor_id" name="doctor_id" class="form-select mt-1 block w-full" required>
                                ...
                                @foreach($doctors as $doctor)
                                    <option value="{{ $doctor->id }}" {{ old('doctor_id') == $doctor->id ? 'selected' : '' }}>
                                        Dr. {{ $doctor->user->name }} ({{ $doctor->department->name ?? 'General' }})
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-6">
                            ...
                            <x-text-input id="admission_date" class="block mt-1 w-full form-control" type="datetime-local" name="admission_date" :value="old('admission_date', now()->format('Y-m-d\TH:i'))" required />
                            <x-input-error :messages="$errors->get('admission_date')" class="mt-2" />
                        </div>

                        <div class="col-md-12">
                            <x-input-label for="notes" :value="__('Admission Notes / Reason')" />
                            <textarea id="notes" name="notes" class="form-control mt-1 block w-full" rows="4" placeholder="Enter reason for admission and initial notes...">{{ old('notes') }}</textarea>

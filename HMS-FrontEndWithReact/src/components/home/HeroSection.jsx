...
import { ArrowRight, Calendar, Phone, Activity, HeartPulse, Stethoscope, Star, Users, Truck, CheckCircle, Clock, Shield, Sparkles, TrendingUp, Award, MessageCircle, Video } from 'lucide-react';
          ...
          <div className="relative w-full h-full flex items-center justify-center">
            ...
            <div className="relative w-full max-w-md">
              ...
              <div className="relative bg-white dark:bg-secondary-800 border border-secondary-200 dark:border-secondary-700 rounded-3xl p-8 transition-all duration-300 hover:shadow-xl hover:scale-[1.01]">
                ...
                <div className="flex items-center justify-between mb-6">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 className="text-secondary-900 dark:text-white font-semibold text-lg">Book Appointment</h3>
                      <p className="text-secondary-500 dark:text-secondary-400 text-sm">Select your preferred time</p>
                    </div>
                  </div>
                ...
                <div className="bg-secondary-50 dark:bg-secondary-900/50 rounded-2xl p-4 mb-4">
                  <div className="grid grid-cols-7 gap-2 mb-3">
                    {['S', 'M', 'T', 'W', 'T', 'F', 'S'].map((day, i) => (
                      <div key={i} className="text-center text-secondary-400 dark:text-secondary-500 text-xs font-medium">{day}</div>
                    ))}
                  </div>
                  <div className="grid grid-cols-7 gap-2">
                    {[...Array(28)].map((_, i) => {
                      const isSelected = i === 15;
                      const isAvailable = [8, 9, 10, 15, 16, 17, 22, 23].includes(i);
                      return (
                        <div
                          key={i}
                          className={`aspect-square rounded-lg flex items-center justify-center text-sm transition-colors duration-200
                            ${isSelected ? 'bg-primary-600 text-white font-bold shadow-lg shadow-primary-900/20' :
                              isAvailable ? 'bg-secondary-200 dark:bg-secondary-600 text-secondary-700 dark:text-white hover:bg-primary-100 dark:hover:bg-primary-900/30 cursor-pointer' :
                  ...
                  {['9:00 AM', '11:00 AM', '2:00 PM', '4:00 PM'].map((time, i) => (
                    <div
                      ...
                      className={`px-4 py-2 rounded-xl text-sm font-medium transition-all duration-200 cursor-pointer ${
                        i === 1 ? 'bg-primary-600 text-white shadow-lg shadow-primary-900/20' : 'bg-secondary-100 dark:bg-secondary-700 text-secondary-600 dark:text-secondary-300 hover:bg-primary-100 dark:hover:bg-primary-900/30 hover:text-primary-600 dark:hover:text-primary-400'
                      }`}
                    >
                      {time}
                    </div>
              ...
              <div className="absolute -top-4 -right-4 bg-white dark:bg-secondary-800 p-3 rounded-2xl flex items-center gap-2 border border-secondary-100 dark:border-secondary-700">
                ...
                <span className="text-sm font-semibold text-secondary-700 dark:text-secondary-200">Available Today</span>
              </div>
          ...
          <div className="relative w-full h-full flex items-center justify-center">
            ...
            <div className="relative w-full max-w-md">
              ...
              <div className="relative h-96 group/stack">
                {[
                  { name: 'Dr. Sarah Chen', specialty: 'Cardiology', rating: 4.9, patients: '2.5k', color: 'bg-primary-600', icon: HeartPulse },
                  { name: 'Dr. Michael Ross', specialty: 'Neurology', rating: 4.8, patients: '1.8k', color: 'bg-teal-600', icon: Activity },
                  { name: 'Dr. Emily White', specialty: 'Pediatrics', rating: 5.0, patients: '3.2k', color: 'bg-indigo-600', icon: Stethoscope }
                ].map((doctor, i) => (
                   <div
                    key={i}
                    ...
                    onMouseLeave={(e) => e.currentTarget.style.zIndex = String(i + 1)}
                  >
                    <div className="flex items-start gap-4">
                      {/* Avatar */}
                      <div className={`${doctor.color} p-4 rounded-2xl`}>
                        <doctor.icon className="w-8 h-8 text-white" />
                      </div>

                      {/* Info */}
                      <div className="flex-1">
                        <h4 className="text-secondary-900 dark:text-white font-bold text-lg mb-1">{doctor.name}</h4>
                        <p className="text-secondary-500 dark:text-secondary-400 text-sm mb-3">{doctor.specialty}</p>

                        {/* Stats */}
                        <div className="flex items-center gap-4">
                          <div className="flex items-center gap-1">
                            <Star className="w-4 h-4 text-yellow-400 fill-yellow-400" />
                            <span className="text-secondary-900 dark:text-white font-semibold text-sm">{doctor.rating}</span>
                          </div>
              ...
              <div className="absolute -bottom-6 -left-6 bg-white dark:bg-secondary-800 border border-secondary-200 dark:border-secondary-700 px-4 py-2 rounded-xl flex items-center gap-2">
                ...
                <div>
                  <div className="text-secondary-900 dark:text-white font-bold text-lg">50+</div>
                  <div className="text-secondary-500 dark:text-secondary-400 text-xs">Specialists</div>
                </div>
              </div>
            </div>
          </div>
              ...
              <div className="bg-white dark:bg-secondary-800 border border-secondary-200 dark:border-secondary-700 rounded-3xl p-8 transition-all duration-300 hover:shadow-2xl hover:-translate-y-1">
                ...
                <div className="flex items-center justify-between mb-6">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 className="text-secondary-900 dark:text-white font-bold text-xl">Emergency Care</h3>
                ...
                <div className="grid grid-cols-2 gap-3 mb-6">
                  {[
                    { label: 'Response Time', value: '<5 min', icon: Clock, color: 'text-teal-500 dark:text-teal-400' },
                    { label: 'Ambulances', value: '12', icon: Truck, color: 'text-rose-500 dark:text-rose-400' },
                    { label: 'Success Rate', value: '99.8%', icon: TrendingUp, color: 'text-green-500 dark:text-green-400' },
                    { label: 'On Duty', value: '24/7', icon: Shield, color: 'text-purple-500 dark:text-purple-400' }
                  ].map((stat, i) => (
                    <div
                      key={i}
                    ...
                    >
                      <stat.icon className={`w-6 h-6 mb-2 ${stat.color} transition-transform duration-300 group-hover:scale-110`} />
                      <div className="text-secondary-900 dark:text-white font-bold text-2xl mb-1">{stat.value}</div>
                      <div className="text-secondary-500 dark:text-secondary-400 text-xs">{stat.label}</div>
                    </div>
                  ))}
                </div>
...
const Button = ({ children, variant = 'primary', size = 'md', className = '', ...props }) => {
  ...
  const variants = {
    ...
    outline: 'border-2 border-secondary-900 text-secondary-900 hover:bg-secondary-900 hover:text-white dark:border-white dark:text-white dark:hover:bg-white dark:hover:text-secondary-900'
  };
  const sizes = {
    md: 'px-6 py-3',
...
const slides = [
  {
    ...
    subtitle: 'Experience world-class medical care with our integrated digital ecosystem. Expert doctors, state-of-the-art facilities, and compassionate care.',
    ctaPrimary: { text: 'Book Appointment', icon: Calendar, path: '/portal/login' },
    ctaSecondary: { text: 'Our Services', icon: ArrowRight, path: '/services' },
  },
  {
    id: 2,
    illustrationType: 'doctors',
    title: 'Expert Doctors You Can Trust',
    subtitle: 'Connect with top specialists across various disciplines. Your health is our priority, and we are here to support you every step of the way.',
  ...
  },
  {
    ...
    subtitle: 'We utilize the latest diagnostic and treatment technologies to ensure accurate results and the best possible outcomes for our patients.',
    ctaPrimary: { text: 'Emergency Help', icon: Phone, path: '/contact' },
    ctaSecondary: { text: 'Patient Portal', icon: ArrowRight, path: '/portal/login' },
  }
];
        ...
        >
          <div className="w-full h-full flex items-center justify-center px-4 sm:px-6 xl:px-8 py-20 xl:py-0 overflow-y-auto xl:overflow-hidden">
            <div className="max-w-7xl mx-auto w-full grid grid-cols-1 xl:grid-cols-2 gap-12 items-center min-h-min xl:h-full mt-16 xl:mt-0">
              ...
              {/* Content Column */}
              <div className="flex flex-col justify-center z-20 text-center xl:text-left order-1 xl:order-1 pt-10 xl:pt-0">
                <motion.div
                ...
                >
                  ...
                  <div className="flex flex-col sm:flex-row gap-4 justify-center xl:justify-start">
                    <Link to={slides[slideIndex].ctaPrimary.path} className="w-full sm:w-auto">
                      <Button className="w-full sm:w-auto px-6 py-3 text-sm sm:text-base sm:px-8 sm:py-4 sm:text-lg hover:-translate-y-1">
                        {React.createElement(slides[slideIndex].ctaPrimary.icon, { className: "w-5 h-5 mr-2" })}
                        {slides[slideIndex].ctaPrimary.text}
                      </Button>
                    </Link>
                    <Link to={slides[slideIndex].ctaSecondary.path} className="w-full sm:w-auto">
                      <Button variant="outline" className="w-full sm:w-auto px-6 py-3 text-sm sm:text-base sm:px-8 sm:py-4 sm:text-lg hover:-translate-y-1">
                        {slides[slideIndex].ctaSecondary.text}
                        {React.createElement(slides[slideIndex].ctaSecondary.icon, { className: "w-5 h-5 ml-2" })}
                      </Button>
                    </Link>
                  </div>
                </motion.div>
              ...
              <div className="relative h-[400px] sm:h-[500px] xl:h-[600px] flex items-center justify-center order-2 xl:order-2 pb-10 xl:pb-0 mt-8 xl:mt-0">
                <GraphicalIllustration type={slides[slideIndex].illustrationType} />
              </div>

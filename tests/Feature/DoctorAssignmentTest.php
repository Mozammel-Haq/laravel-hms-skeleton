...
class DoctorAssignmentTest extends TestCase
{
    ...
    public function test_clinic_admin_can_assign_doctor_only_to_own_clinic_and_access_schedule()
    {
        \Illuminate\Database\Eloquent\Model::unguard();
        $clinicA = Clinic::create(['name' => 'A', 'code' => 'A1', 'address_line_1' => 'A', 'city' => 'C', 'country' => 'C', 'currency' => 'USD', 'timezone' => 'UTC']);
        $clinicB = Clinic::create(['name' => 'B', 'code' => 'B1', 'address_line_1' => 'B', 'city' => 'C', 'country' => 'C', 'currency' => 'USD', 'timezone' => 'UTC']);
        ...
        $user->roles()->attach($role);
        $this->actingAs($user);
        session(['selected_clinic_id' => $clinicA->id]);
        ...
        $response = $this->post(route('doctors.assignment.update', $doctor->id), [
            'clinic_ids' => [$clinicA->id, $clinicB->id],
        ]);

        $response->assertRedirect(route('doctors.assignment'));
        $this->assertTrue($doctor->clinics()->whereKey($clinicA->id)->exists());
        $this->assertTrue($doctor->clinics()->whereKey($clinicB->id)->exists());
    ...
    public function test_clinic_admin_cannot_assign_doctor_to_other_clinic()
    {
        \Illuminate\Database\Eloquent\Model::unguard();
        $clinicA = Clinic::create(['name' => 'A', 'code' => 'A1', 'address_line_1' => 'A', 'city' => 'C', 'country' => 'C', 'currency' => 'USD', 'timezone' => 'UTC']);
        $clinicB = Clinic::create(['name' => 'B', 'code' => 'B1', 'address_line_1' => 'B', 'city' => 'C', 'country' => 'C', 'currency' => 'USD', 'timezone' => 'UTC']);
        ...
        $response->assertRedirect(route('doctors.assignment'));
        $this->assertFalse($doctor->clinics()->whereKey($clinicB->id)->exists());
    }
}

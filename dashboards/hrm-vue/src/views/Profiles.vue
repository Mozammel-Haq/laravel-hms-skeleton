...
const departmentFilterOptions = computed(() => {
  ...
  staffList.value.forEach((s) => {
    ...
    if (d && !map.has(d.id)) map.set(d.id, { id: d.id, name: d.name });
  });
  return Array.from(map.values());
});

const designationFilterOptions = computed(() => {
  if (designations.value?.length) return designations.value;
  const map = new Map();
  staffList.value.forEach((s) => {
...
onMounted(() => {
  fetchStaff();
  // Fetch options (if allowed) to enrich filters; fallback to deriving from staffList
